...
import static com.octo.octo_beat_plugin.core.device._enum.RequestCode.BB_REQ_SSL_CONN_CONFIG;
...
public class ConfigureSSLCommand {
    private static final String TAG = "ConfigureSSLCommand";
    public static byte[] response(int response) {
        if (response != BB_RSP_ERR_PARAM.value &&
                response != BB_RSP_ERR_PERMISSION.value &&
                response != BB_RSP_OK.value) {
        ...
        jsonArray.put(BB_REQ_SSL_CONN_CONFIG.value);
