...
public class TestReceiveTcpData {
    private static final String TAG = "TestReceiveTcpData";
...
//        test_response_06();
    ...
    public static void test_response_01() {
        byte[] bytes = ReadReceiveDataCmd.response(BB_RSP_OK.value, new byte[]{1, 2, 3, 4});
...
//        byte[] bytes = HandShakeCmd.response(BB_RSP_ERR_PARAM);
...
//        byte[] bytes = HandShakeCmd.response(BB_RSP_ERR_PACKET_VER);
...
//        byte[] bytes = HandShakeCmd.response(BB_RSP_ERR_API_VER);
...
//        byte[] bytes = HandShakeCmd.response(BB_RSP_ERR_FULL_DEVICE);
...
//        byte[] bytes = HandShakeCmd.response(0001);
